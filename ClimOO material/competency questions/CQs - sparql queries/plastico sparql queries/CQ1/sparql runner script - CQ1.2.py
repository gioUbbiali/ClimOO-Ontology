from rdflib import Graph
import csv

# -----------------------------
# RDF files to load
# -----------------------------
rdf_files = [
    "/Users/f.u./Desktop/ClimOO-Ontology/ClimOO material/competency questions/CQs - sparql queries/plastico sparql queries/CQ1/climoo-plastico-inferred-hierarchies-2026-09-11.ttl"
    
    # add more files here
]
# This script has been run on Giorgio A. Ubbiali's local version of PlasticO. Please ensure you change the IRI to your local version.

# -----------------------------
# Create graph and load files
# -----------------------------
g = Graph()

for file in rdf_files:
    print(f"Loading {file}...")
    
    # rdflib often guesses format, but explicit is safer:
    if file.endswith(".ttl"):
        g.parse(file, format="turtle")
    elif file.endswith(".rdf") or file.endswith(".xml"):
        g.parse(file, format="xml")
    elif file.endswith(".nt"):
        g.parse(file, format="nt")
    else:
        g.parse(file)

print(f"Loaded {len(g)} triples")

# -----------------------------
# SPARQL query
# -----------------------------
query = """
PREFIX Clim00: <https://www.w3id.org/climoo/climoo-plastico#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
PREFIX skos: <http://www.w3.org/2004/02/skos/core#>

SELECT ?subClass ?label ?definition
WHERE {
  ?subClass rdfs:subClassOf+ Clim00:ClimOO_0000253 .
  OPTIONAL { ?subClass rdfs:label ?label }
  OPTIONAL { ?subClass skos:definition ?definition }
  
}
ORDER BY ?label
"""

# -----------------------------
# Run query
# -----------------------------
results = g.query(query)

# -----------------------------
# Export to CSV
# -----------------------------
output_file = "results.csv"

with open(output_file, "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)

    # write header
    writer.writerow(results.vars)

    # write rows
    for row in results:
        writer.writerow([str(value) for value in row])

print(f"Results written to {output_file}")